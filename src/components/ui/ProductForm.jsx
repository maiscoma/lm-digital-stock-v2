...
export const ProductForm = ({ isOpen, onClose, initialData = null }) => {
    const [formData, setFormData] = useState({ name: '', sku: '', stock: 0, salePrice: '' });
    ...
    useEffect(() => {
        if (isOpen) {
            if (isEditMode) {
                ...
                setFormData({ name: '', sku: '', stock: 0, salePrice: '' });
            }
            setError('');
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        ...
                        <Button type="submit" variant="primary" disabled={loading}>
                            {loading ? <span className="flex items-center gap-2"><CgSpinner className="animate-spin" /> Guardando...</span> : (isEditMode ? 'Guardar Cambios' : 'Crear Producto')}
                        </Button>
                    </div>
                </form>
            </div>
